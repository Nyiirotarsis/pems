...
const statusConfig: Record<DeviceStatus, { color: string; label: string; icon: React.ElementType, pieColor: string; }> = {
    pending: { color: "bg-yellow-100 text-yellow-800 border-yellow-200", label: "Pending", icon: CircleHelp, pieColor: "hsl(var(--chart-3))" },
    verified: { color: "bg-blue-100 text-blue-800 border-blue-200", label: "Verified", icon: ShieldCheck, pieColor: "hsl(var(--chart-1))" },
    approved: { color: "bg-green-100 text-green-800 border-green-200", label: "Approved", icon: ShieldCheck, pieColor: "hsl(var(--chart-2))" },
    blocked: { color: "bg-red-100 text-red-800 border-red-200", label: "Blocked", icon: ShieldAlert, pieColor: "hsl(var(--chart-5))" },
};
...
export default function ScanningDevicesPage() {
    ...
    const summaryStats = React.useMemo(() => {
        ...
        const activeDevices = devices.filter(d => new Date(d.lastUsedAt) > thirtyDaysAgo).length;
        const idleDevices = totalDevices - activeDevices;
        const totalScans = devices.reduce((acc, d) => acc + d.scanCount, 0);
        ...
        const deviceStatusChartData = Object.entries(deviceStatusCounts).map(([name, value]) => ({ name, value }));

        const scansPerDeviceChartData = devices
            .filter(d => d.scanCount > 0)
            .map(d => ({ name: d.deviceName, scans: d.scanCount }))
        ...
        return {
            ...
            idleDevices,
            totalScans,
            blockedDevices: deviceStatusCounts['blocked'] || 0,
            deviceStatusChartData,
            scansPerDeviceChartData,
        };
    }, [devices]);
                 ...
                 <div className="space-y-6">
                    ...
                    <div className="flex justify-between items-start">
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                <h1 className="font-headline text-3xl font-semibold">Device Scanning Dashboard</h1>
                                <p className="text-muted-foreground">Monitor and manage all scanning devices.</p>
                            </div>
                        </div>
                    ...
                    <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
                        <Card>
                            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2"><CardTitle className="text-sm font-medium">Total Devices</CardTitle><Smartphone className="h-4 w-4 text-muted-foreground" /></CardHeader>
                            <CardContent><div className="text-2xl font-bold">{summaryStats.totalDevices}</div></CardContent>
                        </Card>
                        <Card>
                            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2"><CardTitle className="text-sm font-medium">Active Devices</CardTitle><Activity className="h-4 w-4 text-muted-foreground" /></CardHeader>
                            <CardContent><div className="text-2xl font-bold">{summaryStats.activeDevices}</div><p className="text-xs text-muted-foreground">Used in last 30 days</p></CardContent>
                        </Card>
                        <Card>
                            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2"><CardTitle className="text-sm font-medium">Idle Devices</CardTitle><Clock className="h-4 w-4 text-muted-foreground" /></CardHeader>
                            <CardContent><div className="text-2xl font-bold">{summaryStats.idleDevices}</div><p className="text-xs text-muted-foreground">Inactive for 30+ days</p></CardContent>
                        </Card>
                        <Card>
                            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2"><CardTitle className="text-sm font-medium">Total Scans</CardTitle><ScanLine className="h-4 w-4 text-muted-foreground" /></CardHeader>
                            <CardContent><div className="text-2xl font-bold">{summaryStats.totalScans.toLocaleString()}</div><p className="text-xs text-muted-foreground">All time</p></CardContent>
                        </Card>
                        <Card>
                            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2"><CardTitle className="text-sm font-medium">Rejected / Blocked</CardTitle><ShieldAlert className="h-4 w-4 text-muted-foreground" /></CardHeader>
                            <CardContent><div className="text-2xl font-bold">{summaryStats.blockedDevices}</div></CardContent>
                        </Card>
                        <Card>
                            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2"><CardTitle className="text-sm font-medium">Removed Devices</CardTitle><XOctagon className="h-4 w-4 text-muted-foreground" /></CardHeader>
                            <CardContent><div className="text-2xl font-bold">0</div><p className="text-xs text-muted-foreground">Archived or deleted</p></CardContent>
                        </Card>
                    </div>
                    ...
                    <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
                        <Card className="lg:col-span-2">
                            <CardHeader><CardTitle>Scans Per Device (Top 10)</CardTitle><CardDescription>Total scans recorded by the most active devices.</CardDescription></CardHeader>
                            <CardContent>
                                <ResponsiveContainer width="100%" height={250}>
                                    <RechartsBarChart data={summaryStats.scansPerDeviceChartData}>
                                        <CartesianGrid vertical={false} />
                                        <XAxis dataKey="name" stroke="#888888" fontSize={12} tickLine={false} axisLine={false} interval={0} angle={-30} textAnchor="end" height={80}/>
                        ...
                        <Card>
                            <CardHeader><CardTitle>Devices by Status</CardTitle><CardDescription>Current status distribution of all devices.</CardDescription></CardHeader>
                            <CardContent className="flex justify-center">
                                <ResponsiveContainer width="100%" height={250}>
                                    <PieChart>
                                        <Tooltip content={<ChartTooltipContent />} />
                                        <Pie data={summaryStats.deviceStatusChartData} dataKey="value" nameKey="name" cx="50%" cy="50%" innerRadius={50} outerRadius={80} paddingAngle={5} label>
                                            {summaryStats.deviceStatusChartData.map((entry, index) => (
                                                <Cell key={`cell-${index}`} fill={statusConfig[entry.name as DeviceStatus].pieColor || COLORS[index % COLORS.length]} />
                                            ))}
                                        </Pie>
                                    </PieChart>
                                </ResponsiveContainer>
                                    ...
                                    {devices.map(device => {
                                        ...
                                        return (
                                            <TableRow key={device.id}>
                                                ...
                                                <TableCell>{device.scanCount.toLocaleString()}</TableCell>
